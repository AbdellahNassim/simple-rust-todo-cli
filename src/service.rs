use crate::storage::{load_todos_from_file, save_todos_to_file, DEFAULT_STORAGE_FILE};
...
pub fn add_todo(title: String, priority: Priority) {
    let mut todos = load_todos_from_file(DEFAULT_STORAGE_FILE);
    let new_id = get_next_id(&todos);
    let todo = Todo::new(new_id, title.clone(), priority.clone());
    todos.push(todo);
    if let Err(e) = save_todos_to_file(&todos, DEFAULT_STORAGE_FILE) {
...
pub fn list_todos(filter: String) {
    let todos = load_todos_from_file(DEFAULT_STORAGE_FILE);
    if todos.is_empty() {
...
mod tests {
    ...
    fn test_get_next_id() {
        ...
        assert_eq!(get_next_id(&todos), 2);
    }

    #[test]
    fn test_priority_parsing() {
        let priority = match "high".to_lowercase().as_str(){
            "low" | "l" => Priority::Low,
